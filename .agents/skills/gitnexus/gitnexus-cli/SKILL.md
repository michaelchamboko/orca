...
Run from the project root. This parses all source files, builds the knowledge graph, writes it to `.gitnexus/`, and generates the `AGENTS.md` context file.
...
**When to run:** First time in a project, after major code changes, or when `gitnexus://repo/{name}/context` reports the index is stale. In Codex, check index freshness after `git commit` and `git merge`; run `analyze` only when needed, to avoid blocking the agent for up to 120s and risking KuzuDB corruption on timeout.
